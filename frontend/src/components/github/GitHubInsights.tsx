...
export const GitHubInsights: React.FC<Props> = ({ username }) => {
  ...
  const isError =
    profileQuery.isError || statsQuery.isError || reposQuery.isError || contributionsQuery.isError;
    ...
    return (
      <div className="bg-red-50 text-red-600 p-6 rounded-xl border border-red-200 flex flex-col items-center justify-center">
        ...
        <p className="text-sm opacity-80 mt-1">
          This could be due to rate limiting or an invalid username.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <ContributionGraph contributions={contributionsQuery.data} isLoading={isLoading} />

      <GitHubStats stats={statsQuery.data} isLoading={isLoading} />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          <RepositoryStats repositories={reposQuery.data} isLoading={isLoading} />
        </div>
        <div className="md:col-span-1">
          <LanguageStats languages={statsQuery.data?.languages} isLoading={isLoading} />
        </div>
      </div>
    </div>
  );
