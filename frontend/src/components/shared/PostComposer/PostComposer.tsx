...
export const PostComposer: React.FC<PostComposerProps> = ({
  ...
  const addMockAttachment = (type: "repository" | "project" | "poll" | "article") => {
    const attachment: Attachment =
      type === "repository"
        ? { type: "repository", id: "mock-repo-123" }
        : type === "project"
          ? { type: "project", id: "mock-project-456" }
          : type === "poll"
            ? {
                type: "poll",
                data: {
  ...
  return (
    <div className="flex flex-col gap-3">
      <MarkdownEditor value={content} onChange={setContent} placeholder={placeholder} rows={4} />
      ...
      <div className="flex items-center justify-between mt-2 flex-wrap gap-2">
        <div className="flex items-center overflow-x-auto pb-1 gap-1 -mb-1 hide-scrollbar">
          ...
          <Button
            ...
            size="icon"
            className="shrink-0 text-muted-foreground hover:text-foreground"
            onClick={() => addMockAttachment("repository")}
          ...
          <Button
            ...
            size="icon"
            className="shrink-0 text-muted-foreground hover:text-foreground"
            onClick={() => addMockAttachment("project")}
          ...
          <Button
            ...
            size="icon"
            className="shrink-0 text-muted-foreground hover:text-foreground"
            onClick={() => addMockAttachment("poll")}
            title="Create Poll"
          >
            <BarChart size={18} />
          </Button>
          <Button
            ...
            size="icon"
            className="shrink-0 text-muted-foreground hover:text-foreground"
            onClick={() => addMockAttachment("article")}
            title="Write Article"
          >
            <FileText size={18} />
          </Button>
