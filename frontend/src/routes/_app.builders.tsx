...
export const Route = createFileRoute("/_app/builders")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover developers by skills, match score and availability." },
    ],
  }),
...
function BuildersPage() {
  ...
  const { data = [] } = useQuery({ queryKey: ["builders", tab], queryFn: tab === "matches" ? buildersService.matches : buildersService.list });

  const filtered = data.filter(
    (b) => b.name.toLowerCase().includes(q.toLowerCase()) || b.skills.some((s) => s.toLowerCase().includes(q.toLowerCase())),
  );
