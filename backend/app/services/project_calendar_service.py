...
class ProjectCalendarService:
    ...
    def get_events(db: Session, project_id: uuid.UUID) -> List[dict]:
        ...
        stmt = select(ProjectCalendarEvent).where(
    ...
    @staticmethod
    def create_event(
        db: Session, project_id: uuid.UUID, event_in: CalendarEventCreate, actor: User
    ...
    @staticmethod
    def delete_event(
        db: Session, project_id: uuid.UUID, event_id: uuid.UUID, actor: User
    ) -> None:
        project = ProjectMilestoneService.get_project_or_404(db, project_id)
        ProjectMilestoneService.require_project_maintainer(db, project, actor)
