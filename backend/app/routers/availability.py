...
def get_my_availability(
    ...
    """
    availability = db.query(UserAvailability).filter(UserAvailability.user_id == current_user.id).first()
    if not availability:
...
def get_user_availability(
    ...
    availability = db.query(UserAvailability).filter(UserAvailability.user_id == user.id).first()
    if not availability:
