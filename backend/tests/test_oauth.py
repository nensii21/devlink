...
def override_github_config(monkeypatch):
    ...
    monkeypatch.setattr("app.core.config.settings.GITHUB_CLIENT_SECRET", "test_client_secret")

def test_github_login_success_new_user(client: TestClient, db_session, override_github_config):
    # Mock token exchange
    mock_post = AsyncMock()
...
def test_github_login_link_existing_account(client: TestClient, db_session, override_github_config):
    ...
    with patch("httpx.AsyncClient.post", new=mock_post):
        with patch("httpx.AsyncClient.get", new=mock_get):
            ...
            assert data["user"]["username"] == "existing_user" # Keeps original username
